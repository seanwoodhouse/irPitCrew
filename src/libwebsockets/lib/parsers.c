...
swallow:
		...
		//^ if token challenge, complete, 
		//  if not token challenge break;




		//// if not token challenge, break
		//// if token challenge, get token then set complete
		...
		if (lws_hdr_total_length(wsi, WSI_TOKEN_VERSION) &&
			atoi(lws_hdr_simple_ptr(wsi, WSI_TOKEN_VERSION)) < 4 &&
			lws_hdr_total_length(wsi, wsi->u.hdr.parser_state) != 8)
			break;
	...
	case WSI_TOKEN_NAME_PART:
		...
		if (lextable[wsi->u.hdr.lextable_pos] < FAIL_CHAR) {
			...
			/* same goes if we have a version#, only get challenge if no version */
			if (wsi->u.hdr.parser_state == WSI_TOKEN_CHALLENGE &&
...
int libwebsocket_rx_sm_00(struct libwebsocket *wsi, unsigned char c)
{
	int n;
	...
	case LWS_RXPS_NEW:
		...
		case 0:
			...
			break;
		default:
			lwsl_warn("lws_rx_sm_00: unknown spec version %d\n",
				wsi->ietf_spec_revision);
			break;
		}
		break;
	...
	case LWS_RXPS_EAT_UNTIL_76_FF:
		...
		if (wsi->u.ws.rx_user_buffer_head != LWS_MAX_SOCKET_IO_BUF)
			break;
	issue:
		//if (wsi->protocol->callback)
		//{
		...
		//		wsi->u.ws.rx_user_buffer_head);
		//}
		eff_buf.token = &wsi->u.ws.rx_user_buffer[LWS_SEND_BUFFER_PRE_PADDING];
		eff_buf.token_len = wsi->u.ws.rx_user_buffer_head;

		if (lws_ext_callback_for_each_active(wsi, LWS_EXT_CALLBACK_PAYLOAD_RX, &eff_buf, 0) < 0)
			return -1;
	...
	unsigned char *buf, size_t len)
{
	...
	// old version
	else if (wsi->ietf_spec_revision == 0)
	{
		n = 0;
		while (wsi->lws_rx_parse_state != LWS_RXPS_PAYLOAD_UNTIL_LENGTH_EXHAUSTED && n < len)
		{
			if (libwebsocket_rx_sm_00(wsi, buf[n++]) < 0)
			{
				return -1;
			}
		}
	...
	{
		lwsl_warn("libwebsocket_interpret_incoming_packet: unknown spec version %d\n",
			wsi->ietf_spec_revision);
	}
